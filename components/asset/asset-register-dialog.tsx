  ...
  async function downloadTemplate() {
    ...
    try {
      ...
      await write({
        ...
        description: [
          '* 표시 컬럼 = 필수입력. 나머지는 자동차등록증·운영 메타 (빈칸 가능).',
          '· 회사코드 = CP01~CP99 (회사정보에 미리 등록되어 있어야 함)',
          '· 차량번호 = 한국 형식 「12가1234」 또는 「123가1234」 — 회사+차량번호 unique 검증',
          '· 차대번호(VIN) = 17자리 영문+숫자 (예: KMHE0000000000000) — 차량번호보다 상위 unique key',
          '· 날짜 (최초등록일·제작연월·등록증발급일 등) = YYYY-MM-DD 형식 (예: 2024-01-15)',
          '· 차종 = 등록증 표기 그대로 (승용자동차 / 승합자동차 / 화물자동차 / 특수자동차)',
          '· 용도 = 자가용 / 영업용 / 관용 중 하나',
          '· 구동방식 = 드롭다운에서 [전륜 / 후륜 / 4륜 / AWD] 중 선택',
          '· 선택옵션 = 콤마 또는 슬래시(/) 구분 (예: 「파노라마선루프, 하이패스, 어라운드뷰」)',
          '· 금액·치수 컬럼은 천단위 콤마 자동',
        ],
  ...
  return (
    <div className="space-y-3" style={{ paddingTop: 8 }}>
      <div style={{ display: 'flex', alignItems: 'center', gap: 8, paddingBottom: 4, borderBottom: '1px solid var(--border)' }}>
        ...
        <h3 style={{ fontSize: 14, fontWeight: 600, margin: 0 }}>자산(차량) 엑셀 일괄 등록</h3>
        <span className="text-weak text-xs">— 자동차등록증 항목 기준</span>
      </div>
      <div className="text-xs" style={{ background: 'var(--bg-card)', padding: 8, borderRadius: 4 }}>
        · ① <strong>양식 다운로드</strong> → 엑셀에서 행마다 자산 작성. 헤더 <strong>「*」 표시는 필수입력</strong>, 나머지는 부가입력 (빈칸 허용)
